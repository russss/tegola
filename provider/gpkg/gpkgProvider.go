...
import (
	...
	fprovider "github.com/terranodo/tegola/provider"
...
func featuresFromQuery(rows *sql.Rows, idFieldname string, geomFieldname string) ([]fprovider.Feature, error) {
	cols, err := rows.Columns()
	if err != nil {
		return nil, err
	}
	...
	var features []fprovider.Feature
	var ftags map[string]interface{}
	var geom tegola.Geometry
	var geomSRID int

	// Constructs a feature array from the rows
	for rows.Next() {
		geom = nil
		err := rows.Scan(valPtrs...)
		if err != nil {
		...
		for i := 0; i < len(cols); i++ {
			...
			case geomFieldname:
				...
				if geomSRID != DefaultSRID {
					log.Debug("SRID %v != %v, trying to convert...", geomSRID, DefaultSRID)
					// We need to convert our points to Webmercator.
					g, err := basic.ToWebMercator(geomSRID, geom)
					if err != nil {
						log.Error(
							"Was unable to transform geometry to webmercator from "+
								"SRID (%v) due to error: %v",
							geomSRID, err)
						return nil, err
					} else {
						log.Debug("...conversion ok")
...
func (p *GPKGProvider) TileFeatures(ctx context.Context, layerName string, tile tegola.TegolaTile) ([]fprovider.Feature, error) {
	log.Debug("GPKGProvider TileFeatures() called for %v", layerName)
	filepath := p.FilePath
	...
	if p.layers[layerName].tablename != "" {
		...
		if layerBBox.DisjointBB(tileBBox) {
			...
			log.Debug(msg)
			return make([]fprovider.Feature, 0), nil
		}
	}
	...
	features, err := featuresFromQuery(rows, idFieldname, geomFieldname)
	if err != nil {
		log.Error("Problem in call to featuresFromQuery(): %v", err)
		return nil, err
	}
...
func NewFeatureProvider(config map[string]interface{}) (fprovider.FeatureProvider, error) {
	log.Info("GPKGProvider NewProvider() called with config: %v\n", config)
	m := dict.M(config)
	filepath, err := m.String("FilePath", nil)
